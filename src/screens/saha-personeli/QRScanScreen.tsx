...
const PRIORITIES: Priority[] = ['NORMAL', 'URGENT', 'LINE_DOWN', 'PLANNED'];
...
export default function QRScanScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Talep oluşturulamadı:', e);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <View style={styles.container}>
        ...
        {product && !notFound && (
          <View style={{ marginTop: spacing.lg }}>
            ...
            {/* Öncelik Seçimi */}
            <Text style={[typography.body, { color: colors.textSecondary, marginTop: spacing.md, marginBottom: spacing.xs }]}>
              Öncelik
            </Text>
            <View style={styles.priorityRow}>
              {PRIORITIES.map((p) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.priorityChip,
                    { borderColor: priorityColors[p], backgroundColor: priority === p ? priorityColors[p] : colors.white },
                  ]}
                  onPress={() => setPriority(p)}
                >
                  <Text style={{ color: priority === p ? colors.white : priorityColors[p], fontSize: 12, fontWeight: '600' }}>
                    {priorityLabels[p]}
                  </Text>
                </TouchableOpacity>
              ))}
...
const styles = StyleSheet.create({
  ...
  resultOverlay: { justifyContent: 'center', alignItems: 'center', padding: spacing.md },
  scanButton: {
    marginTop: spacing.md, backgroundColor: colors.blue,

...
export default function RejectRequestScreen() {
  ...
  return (
    <Box style={{ flex: 1 }} background="white">
      {/* Diğer ekranlarla (RequestDetailScreen, IncomingRequestsScreen) tutarlı
          mavi header — önceden bu ekranda hiç header yoktu. */}
      <Box
        background="danger"
        style={{ paddingTop: insets.top + spacing.md, paddingHorizontal: spacing.md, paddingBottom: spacing.md }}
      >
        <Stack direction="row" align="center" gap="md">
          <Pressable onPress={() => navigation.goBack()} background="dangerLight" radius="md" accessibilityLabel="Geri">
            <Ionicons name="chevron-back" size={20} color={colors.danger} />
          </Pressable>
          <Box>
            <Text variant="caption" color="white" style={{ opacity: 0.8, letterSpacing: 1 }}>
              {summary.requestId.toUpperCase()} · {summary.quantity} adet
            </Text>

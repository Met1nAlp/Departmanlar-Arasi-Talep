...
InventoryManager::InventoryManager() {
    ...
    // Mock eşyaları
    mockDatabase.insert("MEPSAN_PRINTER_001",
                        {"MEPSAN_PRINTER_001", "HP Laser Printer", "STOKTA VAR", ""});
    mockDatabase.insert("MEPSAN_LAPTOP_002",
                        {"MEPSAN_LAPTOP_002", "Lenovo Thinkpad", "KULLANIMDA", "Baris"});
    mockDatabase.insert("MEPSAN_DRILL_003",
                        {"MEPSAN_DRILL_003", "Bosch Darbeli Matkap", "STOKTA YOK", ""});

}

// Envanterden Esya alma kontrolü
bool InventoryManager::checkoutItem(const QString &qrCode, const QString &username) {
    if (mockDatabase.contains(qrCode)) {
        InventoryItem &item = mockDatabase[qrCode];

        if (item.status == "KULLANIMDA") {
            qDebug() << "[ERROR] Esya zaten su kisi tarafindan kullanimda:" << item.assignedTo;
            return false;
        }

        item.status = "KULLANIMDA";
        item.assignedTo = username;
        qDebug() << "[ENVANTER] Checkout Basarili -> Esya:" << item.name << "| Tarafindan:" << username;
        return true;
    }
    return false;
}
...
bool InventoryManager::checkinItem(const QString &qrCode) {
    if (mockDatabase.contains(qrCode)) {
        ...
        item.assignedTo = "";
        qDebug() << "[ENVANTER] Checkin Basarili -> Esya:" << item.name << "Tekrardan stokta";
        return true;
    }
    return false;
}
